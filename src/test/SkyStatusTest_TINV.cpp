...
int main()
{
  ...
  vector<Pressure> v_ProfilePressureLevels;
  ...
  if (fp != 0) {
    ...
    while (feof(fp)==0){
      fgrow = fgets( aRow, STRLEN, fp );
      token = strtok(aRow,",");  v_ProfileHeightLevels.push_back(Length(atof(token),"m"));
      // cout << "boundary" << token << endl;
      token = 0; token = strtok(token," "); v_ProfileTemperatureLevels.push_back(Temperature(atof(token),"K"));
      // cout << "temperature" << token << endl;
      // token = 0; token = strtok(token,","); v_ProfilePressureLevels.push_back(Pressure(atof(token),"mb"));
      // token = 0; token = strtok(token," "); v_ProfileWaterVaporLevels.push_back(MassDensity(atof(token),"gm**-3"));
      //      cout << "H2O:" << token << endl;
    }
  }
  fclose( fp );

  //  AtmProfile myProfile(v_ProfileHeightLevels, v_ProfilePressureLevels, v_ProfileTemperatureLevels, v_ProfileWaterVaporLevels);
  ...
  Pressure     Pstep(   3.0,"mb");     // Primary pressure step (5.0 mb)
  double   PstepFact=        1.05;     // Pressure step ratio between two consecutive layers (1.1(
  ...
  AtmProfile myProfile( Alt, P, T, TLR, H, WVL, Pstep, PstepFact,  topAtm, atmType, v_ProfileHeightLevels, v_ProfileTemperatureLevels);
  ...
  cout << " SkyStatusTest actual profile:" << endl;
  cout << "!  P0       P       P1        T0        T         T1        H0          H1       lyr tck    H2O_0     H2O_1      H2O" << endl;
  cout << "! (mb)     (mb)    (mb)      (K)       (K)       (K)       (m)         (m)         (m)     (g m^-3)  (g m^-3)  (g m^-3)" << endl;
  ...
  for(unsigned int i=0; i<myProfile.getNumLayer(); i++){
    cout << myProfile.getLayerBottomPressure(i).get("mb")  << " "
         ...
         << myProfile.getLayerBottomTemperature(i).get("K")   << "  "
	 << myProfile.getLayerTemperature(i).get("K")   << "  "
         << myProfile.getLayerTopTemperature(i).get("K")   << "  "
	 << myProfile.getLayerBottomHeightAboveSeaLevel(i).get("m")   << "   "
	 << myProfile.getLayerTopHeightAboveSeaLevel(i).get("m")   << "   "
	 << myProfile.getLayerThickness(i).get("m")   << "   "
      //	 << myProfile.getLayerWaterVaporMassDensity(i).get("kgm**-3")  << " "
	 << myProfile.getLayerBottomWaterVaporMassDensity(i).get("gm**-3")  << "    "
	 << myProfile.getLayerTopWaterVaporMassDensity(i).get("gm**-3")  << "    "
	 << myProfile.getLayerWaterVaporMassDensity(i).get("gm**-3")  << " "
      // << myProfile.getLayerCO(i).get("m**-3")   << " "
      //   << myProfile.getLayerO3(i).get("m**-3")   << " "
      //   << myProfile.getLayerN2O(i).get("m**-3")  <<
	 << endl;
  }
